...
const SevensaListItem: React.FC<{ children: React.ReactNode; isSelected: boolean; onSelect: () => void }> = ({ children, isSelected, onSelect }) => (
  ...
  >
    <div className="w-8 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
      <Checkbox checked={isSelected} onCheckedChange={onSelect} aria-label="Select item" />
    </div>
  ...
  const renderControls = () => (
    <div className="flex items-center space-x-4">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          {columns.filter(c => c.sortable).map(column => (
            <DropdownMenuItem key={String(column.key)} onClick={() => handleSort(column.key)}>
              <div className="flex items-center justify-between w-full">
                <span>{column.title}</span>
                {sortBy?.key === column.key && (
                  <span className="ml-2 text-xs text-sevensa-blue">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => handleGroupBy(null)}>
            <div className="flex items-center justify-between w-full">
              <span>None</span>
              {!groupBy && <Check className="h-4 w-4 text-sevensa-blue" />}
            </div>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          {columns.filter(c => c.groupBy).map(column => (
            <DropdownMenuItem key={String(column.key)} onClick={() => handleGroupBy(column.key)}>
              <div className="flex items-center justify-between w-full">
                <span>{column.title}</span>
                {groupBy === column.key && <Check className="h-4 w-4 text-sevensa-blue" />}
              </div>
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      ...
      {hasSelection && bulkActions.length > 0 && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="default" className="bg-sevensa-blue hover:bg-sevensa-blue/90 text-white">
              {selectedItems.size} Selected
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            {bulkActions.map(action => (
              <DropdownMenuItem key={action.id} onClick={() => action.onAction(selectedData as T[])}>
