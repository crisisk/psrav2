...
export async function GET(request: NextRequest) {
  ...
  const cached = await cacheService.getCachedHsCodes<HsCodeResponse>(cacheKey);
  if (cached) {
...
async function resolveTaricFallback(search: string | null, chapter: string | null): Promise<HsCodeResponse | null> {
  ...
  if (taricResults?.commodities?.length) {
    const hsCodes = taricResults.commodities
      .filter((commodity) => (chapter ? commodity.code.startsWith(chapter) : true))
      .map((commodity) => ({
        code: commodity.code,
