...
export default function ImportPage() {
  ...
  const handleImport = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!file) {
      setError('Selecteer eerst een CSV-bestand met HS39/40 materiaalregels.');
      return;
    }

    if (!file.name.toLowerCase().endsWith('.csv')) {
      setError('Het bestand moet de extensie .csv hebben.');
      return;
    }
    ...
    reader.onload = () => {
      ...
      if (!parsedSummary) {
        setError('Kon geen geldige rijen in het CSV-bestand vinden. Controleer de delimiter en headers.');
        return;
      }
    ...
    reader.onerror = () => {
      setIsProcessing(false);
      setError('Het bestand kon niet worden gelezen. Probeer opnieuw of controleer de permissies.');
    };
